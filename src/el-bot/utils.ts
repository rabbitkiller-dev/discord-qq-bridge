...
export async function generateQQMsgContentAvatar(url: string, useCache?: boolean): Promise<MessageType.Image> {
	...
	const stream = fs.createWriteStream(path.join(imageDiscordAvatarCacheDir, path.basename(filePath)));
	stream.write(canvas.toBuffer());
	stream.close();
	const relativePath = path.join("../", imageDiscordAvatarCacheDir.replace(cacheDir, "cache"), path.basename(filePath));
	console.log("relativePath2: ", relativePath);
	return MiraiMessage.Image(null, null, relativePath.replace(/\\/g, "/"));
}

export async function remoteImageToLocal(avatar: string, useCache = true): Promise<string> {
